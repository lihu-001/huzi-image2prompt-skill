...
- 覆盖写实照片、插画、海报和含文字图片
- 提取主体、构图、环境、风格、光线、分区色相/明度/饱和度、材质微结构、遮挡关系、服装/物体结构、镜头观感与宽高比
- 先建立厂商无关的 Canonical Visual Spec，显式记录对象关系、前后层级和 `P0 identity / P1 similarity / P2 flexible` 复现优先级
- 按骨架与轴线、比例与锚点、负形与遮挡、三值明暗、空间尺度、色彩标定与材质纹理的顺序建立 Reconstruction Spec；结构未通过时触发纹理门禁
...
- 每个代码块包含完整正向描述和反义提示，一次复制、一次粘贴即可出图；GPT 与 Grok 共享画面事实，但分别按平台语法编译，不直接混用同一段成品 Prompt
- 原始像素尺寸与生成比例分开管理：GPT/Grok 开头写方向、保真比例和原始尺寸参考，Midjourney 的 `--ar` 使用与原图一致的常用比例或低误差紧凑比例
- 对织物颗粒、粗糙度、光泽、厚度、褶皱和拉伸进行显式描述，避免只写笼统的“真实材质”
- 对主要颜色区分别记录色相、相对明度、相对饱和度和色度排序，保留原图的高饱和锚点，避免只对上色相却整体发灰
- 区分局部高色度与全局偏色；每个区域都按原图分别保持鲜艳、克制或中性的色彩状态
...
- Midjourney 版本自动附加保真且紧凑的 `--ar`、`--style raw` 和相关 `--no`；例如 `405 × 793` 使用近似误差低于 0.25% 的 `24:47`，而不是机械写 `405:793` 或强行改成不一致的常用比例
...
Markdown 文件包含图片来源、精确原始尺寸、规范化生成比例、通过 `![原图](<assets/...>)` 相对路径嵌入的原图预览，以及以下六个可独立复制的平台专用代码块：
...
内部先建立厂商无关的 Canonical Visual Spec，记录画布、主体、几何分区、空间和遮挡关系、前后景、光色、材质、文字，以及 P0/P1/P2 复现优先级。随后以 Reconstruction Spec 锁定骨架、比例、负形、三值明暗和空间层级，最后才放行纹理；这些内部表示不会作为“原版”“通用”或“自然语言”Prompt 输出。原始尺寸会先规范化为目标生成比例：与 `16:9`、`21:9`、`4:5`、`1:2` 等常用比例的相对误差不超过 0.25% 时采用常用表达，否则保留精确约分结果或使用误差不超过 0.25%、两项均不大于 99 的紧凑近似，避免为追求熟悉数字而改变原图画布。最后由三个平台 adapter 分别编译：GPT 使用完整关系句，Grok 使用紧凑的结构分区指令；两者都在开头携带方向、生成比例和可靠的原始尺寸参考。Midjourney 把完整取景和 P0 几何放在高显著局部细节之前，并在末尾附加：

```text
--ar [规范化生成比例] --style raw --no [反义提示]
```

不会单独输出 Negative Prompt 或“不确定项”。生成前会建立骨架比例、负形遮挡、材质、结构、可见性、构图空间、明暗色彩七类高漂移画像；明暗色彩画像会按区域记录色相、明度、饱和度和色度排序，并要求所有平台版本保持一致。模型偏差只在 adapter 中基于用户对比或回归证据进行场景相关补偿。用户提供生成结果时，每轮只修正一到两个最高权重误差并锁定已正确 P0；用户明确指出的欠饱和等漂移会直接进入本轮修正。除非用户明确要求生成，否则不会自动出图，自动纠偏最多三轮。若图片存在会显著改变结果、且用户能够判断的关键歧义，技能会先询问一个确认问题。多张图片默认分别生成 Markdown 文件。
...
evals/canvas-aspect-ratio-regression.json  原始尺寸、常用比例吸附、紧凑比例和三平台画布输出回归规范
evals/reconstruction-hierarchy-regression.json  骨架优先重建、纹理门禁与纠偏回归规范
evals/color-saturation-regression.json  分区色相/明度/饱和度、局部色度锚点与色彩保真门禁回归规范
...
图片反推只能重建视觉特征，无法仅凭成图可靠恢复原始模型、LoRA、种子、采样器、步数、CFG 或后期处理参数。GPT/Grok Prompt 中的原始尺寸是构图参考；平台是否输出精确像素仍取决于其界面或 API 的尺寸控制。生成模型对精确文字的支持也不稳定，复杂排版通常需要后期处理。
